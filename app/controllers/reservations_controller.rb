class ReservationsController < ApplicationController
  ...
  def submit_reservations
    @reservations = Reservation.joins('INNER JOIN students on students.id = reservations.student_id INNER JOIN users on users.id = students.user_id').where('users.id = ?', current_user[:id])
